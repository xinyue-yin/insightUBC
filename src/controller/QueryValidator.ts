...
export class QueryValidator {
    ...
    public validateOrder(order: any): boolean {
        ...
        if (this.validJson(order)) {
            return this.validateOrderObject(order);
        } else {
            return this.validateOrderString(order);
        }
    }

    private validateOrderString(input: any): boolean {
        if (this.columns.includes(input)) {
            if (this.isValidIdFieldOfKind(input)) {
                return true;
            } else {
                return this.isValidApplyKey(input);
            }
        } else {
    ...
    public validateSolutionSize(solution: any[], desiredSize: number) {
        if (solution.length > desiredSize) {
            let errMsg = "The result is too big. Only queries with a maximum of 5000 results are supported.";
            throw new ResultTooLargeError(errMsg);
        }
    }
