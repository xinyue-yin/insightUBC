...
class AddRooms {
    ...
    parseRooms() {
        return new Promise((resolve, reject) => {
            this.parseIndex().then(() => {
                this.zip.folder("rooms").forEach((relativePath, file) => {
    ...
    parseIndex() {
        return new Promise((resolve, reject) => {
            this.zip.file("rooms/index.htm").async("text").then((file) => {
                this.traverseBuildings(this.parse5.parse(file), this.buildingInfos);
            }).then(() => {
        ...
        return new Promise((resolve, reject) => {
            Promise.all(this.folderContents).then((buildings) => {
                ...
                for (let building of buildings) {
                    let validRooms = this.isContainingAtLeastOneRoom(this.parse5.parse(building));
                    if (validRooms > 0) {
                        isValidDataset = true;
                    }
                    i++;
                ...
                if (!isValidDataset) {
                    return reject(new IInsightFacade_1.InsightError("Datasets in " + this.id + " are invalid."));
                }
                else {
                ...
                return reject(new IInsightFacade_1.InsightError("Invalid Dataset"));
            });
        });
    }
    writeToDisk(id) {
    ...
    buildingProperties(array) {
        return new Promise((resolve) => {
            ...
            for (let i = 0; i < array.length; i += 4) {
                let addressURL = encodeURI(array[i + 3]);
            ...
            Promise.all(promiseArray).then((resultArray) => {
                for (let result of resultArray) {
                    if (!result.hasOwnProperty("error")) {
                        ...
                        this.relativePath.push(array[k + 1].substring(2));
    ...
    roomProperties(room, buildingName) {
        let i = 0;
        let length = room.length;
        let building = this.listOfBuildings.filter((element) => {
    ...
    isContainingAtLeastOneRoom(building) {
        let roomInfos = [];
        let name = building.childNodes[6].childNodes[1].childNodes[9].attrs[1].value;
